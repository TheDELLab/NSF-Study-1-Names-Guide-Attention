...
##################################################
# Animation Subroutine - START
##################################################


# Function to animate a screen
def animation_screen(win, orientation, imgs, animation_duration=4):
    ...
    if orientation == 'normal':
        ...
        image = visual.ImageStim(win, image=next(imgs),size=[0.2, 0.3])  # Adjusted size for 'norm' units
        clock = core.Clock()

        # Set starting position of the image
        x_pos = -1.1  # Start slightly to the left of the screen
        y_pos = spline(x.min()) * 0.9  # Adjusted for the 'norm' coordinate system
        image.pos = [x_pos, y_pos]
    
        # Loop through the spline points and move the image stimulus
        while clock.getTime() < animation_duration:
            for t in np.linspace(x.min(), x.max(), num=270):
                x_pos = -1.1 + t * 2.2  # Adjusted for 'norm' units
                y_pos = spline(t) * 0.9  # Adjusted for 'norm' units
                image.pos = [x_pos, y_pos]
                image.draw()
                win.flip()
                if x_pos > 1.1 or clock.getTime() >= animation_duration:  # Check if image has reached the right end
                    break
        ...
        # Set starting position of the image
        x_pos = 1.1  # Start slightly to the right of the screen
        y_pos = spline(x.max()) * 0.9  # Adjusted for the 'norm' coordinate system
        image.pos = [x_pos, y_pos]
        ...
        while clock.getTime() < animation_duration:
            for t in np.linspace(x.min(), x.max(), num=270):
                ...
                image.draw()
                win.flip()
                if x_pos < -1.1 or clock.getTime() >= animation_duration:  # Check if image has reached the left end
                    break
...
def simulate_trial(win):
    ...
    #start practice 
    start_practice_text = visual.TextStim(win, text='Press space bar to start the practice trials', pos=(0,-0.5), height=0.07, color='black')
    ...
    if control_key[0] == 'space':
        ...
        for trialPrac, audioPrac in zip(trial_range_practice, audio_practice):
            ...
            # Create the stimuli
            doll = visual.ImageStim(win, image='./images/Cookie-Monster-smaller.png',pos=(0,0.5))
            number = visual.TextStim(win, text=condPrac.loc[trialPrac, 'Target'], color='black', height=0.5, pos=(0, 0) )
            ...
            if condPrac.loc[trialPrac,'Location'] == 'left':
                left_number = visual.TextStim(win, text=condPrac.loc[trialPrac, 'Target'], color='black', height=0.5, pos=(-0.5, 0))
                right_number = visual.TextStim(win, text=condPrac.loc[trialPrac, 'Foil'], color='black', height=0.5, pos=(0.4, 0))
            else:
                left_number = visual.TextStim(win, text=condPrac.loc[trialPrac, 'Foil'], color='black', height=0.5, pos=(-0.5, 0))
                right_number = visual.TextStim(win, text=condPrac.loc[trialPrac, 'Target'], color='black', height=0.5, pos=(0.4, 0))
...
##################################################
# Psychopy Config - START
##################################################


# open a window
win = visual.Window(fullscr=True, units='norm', color=(1, 1, 1))
# info screen
info = visual.TextStim(win, text='Press Space to Start Trial', pos=(0,-0.4), height=0.1, color='black')
...
# For Checkpoint ( poker coin image trigger )
checkpoint_flag = 0
iterator = 1
##################################################
# Reading Conditions - END
##################################################
...
#start practice 
start_actual_text = visual.TextStim(win, text='Press space bar to start the actual trials', pos=(0,-0.5), height=0.07, color='black')
start_actual_text.draw()
win.flip()
...
if control_key[0] == 'space':
    for trial, audio in zip(trial_range[:1], audio[:1]):
        
    
        # Create the stimuli
        doll = visual.ImageStim(win, image='./images/Cookie-Monster-smaller.png',pos=(0,0.5))
        number = visual.TextStim(win, text=cond.loc[trial, 'Target'], color='black', height=0.5, pos=(0, 0) )
        ...
        if cond.loc[trial,'Location'] == 'left':
            left_number = visual.TextStim(win, text=cond.loc[trial, 'Target'], color='black', height=0.5, pos=(-0.5, 0))
            right_number = visual.TextStim(win, text=cond.loc[trial, 'Foil'], color='black', height=0.5, pos=(0.4, 0))
        else:
            left_number = visual.TextStim(win, text=cond.loc[trial, 'Foil'], color='black', height=0.5, pos=(-0.5, 0))
            right_number = visual.TextStim(win, text=cond.loc[trial, 'Target'], color='black', height=0.5, pos=(0.4, 0))
...
end_message = visual.TextStim(win, text="Saving Data ...", color='black', height=0.02, pos=(0, 0))
end_message.draw()
win.flip()
time.sleep(2)
