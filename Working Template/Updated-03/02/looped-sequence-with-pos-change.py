...
with open('conditionals.csv', newline='') as csvfile:
    ...
    for row in reader:
        ...
        number = visual.TextStim(win, text=row['numbers'], color='black', height=0.35)
        plane = visual.ImageStim(win, image=next(image_list))
        plane.setPos([-1, -1])
        left_number = visual.TextStim(win, text=row['numbers'], color='black', height=0.35, pos=(-0.3, 0))
        right_number = visual.TextStim(win, text=row['foiled'], color='black', height=0.35, pos=(0.3, 0))
