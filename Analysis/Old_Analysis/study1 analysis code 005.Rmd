...
data_005 = read.csv("datavyuExports/export_delayed_match_005_LV.csv")

#change column names to be more understandable

colnames(data_005) <- c("frameNumber", "time", "gazeOrdinal", "gazeOnsetTime", "gazeOffsetTime", "POI_lable", "trialNumber", "trialOnset", "trialOffset", "targetNumber")
...
cleaned_data <- remove_empty_gaze_pos_rows(data_005)

View(cleaned_data)

#export new cleaned file
write.csv(cleaned_data, "cleaned_file_005.csv", row.names = FALSE)


```

```{r}
df <- read.csv("cleaned_file_005.csv")
...
# Read the data
df_basic_summary <- read.csv("cleaned_file_005.csv")
...
# Calculate the total gaze duration for each trial
total_duration_per_trial <- aggregate(duration ~ trialNumber, df_basic_summary, sum)
print(total_duration_per_trial)

# Merge this with the total_duration_per_label_per_trial dataframe to get total duration per trial for each POI_lable
df_merged <- merge(total_duration_per_label_per_trial, total_duration_per_trial, by="trialNumber")

# Compute the percentage
df_merged$percentage <- (df_merged$duration.x / df_merged$duration.y) * 100

# Print the percentage of each POI_lable used in each trial
